...
#ifndef NDN_FILECONSUMER_LOG_TRACER_H
#define NDN_FILECONSUMER_LOG_TRACER_H
 ...
 */
class FileConsumerLogTracer : public SimpleRefCount<FileConsumerLogTracer> {
public:
  /**
   ...
   *          for the lifetime of simulation, otherwise SEGFAULTs are inevitable
   */
  static Ptr<FileConsumerLogTracer>
   ...
   */
  FileConsumerLogTracer(shared_ptr<std::ostream> os, Ptr<Node> node);
   ...
   */
  FileConsumerLogTracer(shared_ptr<std::ostream> os, const std::string& node);

  /**
   * @brief Destructor
   */
  ~FileConsumerLogTracer();
